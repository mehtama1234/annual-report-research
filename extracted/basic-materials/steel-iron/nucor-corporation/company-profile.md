# Nucor Corporation

Date baseline: 2026-08-10

## Identity

- Company: Nucor Corporation
- Ticker: NUE
- Exchange: NYSE
- Sector: Basic Materials
- Industry: Steel & Iron
- Headquarters: Charlotte, North Carolina, United States
- Fiscal year-end: December
- AnnualReports.com page: https://www.annualreports.com/Company/nucor-corporation
- Investor relations page: https://investors.nucor.com/
- SEC / EDGAR reference: https://www.sec.gov/Archives/edgar/data/73309/000119312526071575/nue-20251231.htm

## Coverage status

- Target annual report year: 2025
- Target quarter window:
  - quarter 1: Q2 2026
  - quarter 2: Q1 2026
  - quarter 3: Q4 2025
- Latest annual report collected: yes
- Latest annual filing collected: yes
- Latest quarter collected: yes
- Quarter minus 1 collected: yes
- Quarter minus 2 collected: yes
- Latest earnings call transcript collected: no official transcript saved locally

## Annual report archive

| Year | Document | Source URL | Local path | Notes |
|---|---|---|---|---|
| 2025 | AnnualReports company page | https://www.annualreports.com/Company/nucor-corporation | [company-page-annualreports.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/annualreports/basic-materials/steel-iron/nucor-corporation/company-page-annualreports.html) | Confirms `Basic Materials` / `Steel & Iron` taxonomy and shows AnnualReports still lagged at the `2024 Annual Report and Form 10-K` on `2026-08-10`. |
| 2025 | Nucor annual report PDF | https://s202.q4cdn.com/531038915/files/doc_financials/2026/ar/2025-Nucor-Annual-Report.pdf | [2025-annual-report.pdf](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/company-ir/basic-materials/steel-iron/nucor-corporation/2025-annual-report.pdf) | Official annual report to stockholders for `2025`. |
| 2025 | Nucor Form 10-K HTML | https://www.sec.gov/Archives/edgar/data/73309/000119312526071575/nue-20251231.htm | [2025-10k.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/sec/basic-materials/steel-iron/nucor-corporation/2025-10k.html) | Core annual filing for the year ended `2025-12-31`. |
| 2025 | Nucor `Q4 2025` results exhibit | https://www.sec.gov/Archives/edgar/data/73309/000119312526022733/d20933dex991.htm | [2025-q4-results-exhibit-99-1.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/company-ir/basic-materials/steel-iron/nucor-corporation/2025-q4-results-exhibit-99-1.html) | Official detailed results release for `Q4 2025`. |

## Last three quarters

| Quarter | Earnings release | Filing | Transcript | Key headline |
|---|---|---|---|---|
| Q2 2026 | [2026-q2-results-exhibit-99-1.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/company-ir/basic-materials/steel-iron/nucor-corporation/2026-q2-results-exhibit-99-1.html) | [2026-q2-8k.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/sec/basic-materials/steel-iron/nucor-corporation/2026-q2-8k.html) | none saved locally | Nucor reported `Q2 2026` net earnings attributable to stockholders of `$1.16 billion`, adjusted earnings of `$1.11 billion`, net sales of `$10.40 billion`, and EBITDA of `$2.02 billion`. No `Q2 2026` `10-Q` was yet filed locally as of `Monday, August 10, 2026`. |
| Q1 2026 | [2026-q1-results-exhibit-99-1.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/company-ir/basic-materials/steel-iron/nucor-corporation/2026-q1-results-exhibit-99-1.html) | [2026-q1-10q.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/sec/basic-materials/steel-iron/nucor-corporation/2026-q1-10q.html) | none saved locally | Nucor reported `Q1 2026` net earnings attributable to stockholders of `$743 million`, net sales of `$9.50 billion`, and EBITDA of `$1.51 billion`, with record steel mill shipments. |
| Q4 2025 | [2025-q4-results-exhibit-99-1.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/company-ir/basic-materials/steel-iron/nucor-corporation/2025-q4-results-exhibit-99-1.html) | [2025-q4-8k.html](/home/manishmehta/ui-projects/annual-report-research-energy-buildout/raw/sec/basic-materials/steel-iron/nucor-corporation/2025-q4-8k.html) | none saved locally | Nucor reported `Q4 2025` net earnings attributable to stockholders of `$378 million`, adjusted earnings of `$400 million`, net sales of `$7.69 billion`, and EBITDA of `$918 million`. |

## What changed

- Earnings and sales: `Q4 2025` was softer, with net earnings attributable to Nucor stockholders of `$378 million` on net sales of `$7.69 billion`. The business then improved sharply in `Q1 2026` to `$743 million` of earnings on `$9.50 billion` of sales, and again in `Q2 2026` to `$1.16 billion` of earnings on `$10.40 billion` of sales.
- Segment mix: Nucor remains best read through its three operating segments: steel mills, steel products, and raw materials. In `Q1 2026`, segment earnings before taxes and noncontrolling interests were `$1.128 billion` in steel mills, `$285 million` in steel products, and `$45 million` in raw materials. In `Q2 2026`, those rose to `$1.556 billion`, `$353 million`, and `$146 million`, respectively.
- Steel-cycle dynamics: the key quarter-to-quarter driver was not just volume but price plus mix. `Q4 2025` suffered from lower volumes and margin compression, especially in sheet. `Q1 2026` improved on higher volumes and selling prices. `Q2 2026` improved further on higher realized selling prices, higher volumes, and stronger raw-material pricing.
- Cash and balance sheet: Nucor ended `Q4 2025` with `$2.70 billion` of cash and short-term investments, ended `Q1 2026` with `$2.48 billion`, and ended `Q2 2026` with `$2.69 billion`, while keeping its `$2.25 billion` revolving credit facility undrawn.
- Capital allocation: Nucor continued meaningful shareholder returns while keeping heavy investment behind growth. It returned about `$250 million` to stockholders in `Q1 2026`, about `$479 million` in `Q2 2026`, and roughly `$1.2 billion` during full-year `2025`.
- Capital spending: cash used for capital expenditures increased to `$3.42 billion` in `2025` from `$3.17 billion` in `2024`, and management estimated approximately `$2.50 billion` of capital expenditures for `2026`.
- Operating footprint: Nucor remains a scale domestic steel system, not a single-product steel name. In `2025`, it recycled approximately `20 million` gross tons of scrap steel, sold about `19.848 million` tons to outside customers from the steel mills segment, and ran a business mix where sheet contracts accounted for about `85%` of sheet steel sales.
- Strategic positioning: the company is leaning into a value-added, less-volatile product mix through new projects and expansions, including the West Virginia sheet mill, galvanizing capacity, towers and structures, and other fabricated-steel capabilities tied to data centers, energy, and industrial buildout.
- Management tone: Nucor is framing itself as a domestic industrial winner from supportive U.S. trade policy, strong backlogs, capital investment, and a North America-centered manufacturing base.

## Open questions

- How much of `Q2 2026` strength reflects sustainable through-cycle earnings improvement versus a strong mid-cycle steel-price and policy tailwind?
- Can Nucor’s recent capex wave actually reduce volatility over time by shifting mix toward more value-added and contract-oriented products?
